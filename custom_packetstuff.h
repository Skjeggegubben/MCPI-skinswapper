    ...
    #define PACKET_ID 0xb9 // 0xb8 is the sissy-mod
    ...
    typedef void (*ServerSideNetworkHandler_redistributePacket_t)(uchar *callback, uchar *packet, RakNet_RakNetGUID *guid);
    ...
    typedef void (*RakNetInstance_sendTo_t)(uchar *self, RakNet_RakNetGUID *guid, uchar *packet);
    static uint32_t RakNetInstance_sendTo_vtable_offset = 0x3c;


    //void send_skin_packet(std::string strArg, uchar *guid, uchar *callback);
    void send_skin_packet(std::string strArg, RakNet_RakNetGUID *guid, uchar *callback);
    void redistribute_skin_packet(std::string strArg, RakNet_RakNetGUID *guid, uchar *callback);
