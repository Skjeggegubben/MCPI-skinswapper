...
void send_skin_packet(std::string strArg, RakNet_RakNetGUID *guid, uchar *callback){// Neat function for just shipping off a string in a custom packet
    ...
    if(guid == NULL){
		...
		//server instance is sending packets to clients
		//ServerSideNetworkHandler_redistributePacket(callback, packet, guid);
...
void redistribute_skin_packet(std::string strArg, RakNet_RakNetGUID *guid, uchar *callback){// Neat function for redistribution!
	int intArg = strArg.size(); // Need to get the string length so we can feed it as arg along with the string
    uchar *packet = make_custom_packet(intArg, strArg);	// to the make_custom_packet function :) 
    //uchar *rak_net_instance = *(uchar **) ( get_minecraft() + Minecraft_rak_net_instance_property_offset); // This and next lines just sends it.. 
    if(guid == NULL){
		INFO("you made a boo-boo");
...
static void CustomPacket_handle(CustomPacket *self, RakNet_RakNetGUID *guid, uchar *callback) {
    // Handle it, you can also use args here
    //INFO("The len_arg is: %i", self->len_arg);
	//std::string packetPayload = self->str_arg;
	//if( packetPayload.size() > 60 ) packetPayload  = shortened(packetPayload, 30);
	//INFO("[SKINSWAPPER]: str_arg is: '%s'", packetPayload.c_str() ); //self->str_arg.c_str() );
	parse_skin_packet(self->str_arg, guid, callback);
}
